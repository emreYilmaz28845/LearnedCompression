...
\begin{frame}{Training Objective}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{block}{Training Setup}
      ...
      Adam: \(10^{-4}\) backbone, \(10^{-3}\) SE\\[0.18cm]
      3 freeze modes for SE adapter test
    \end{block}
    ...
    \begin{exampleblock}{Evaluation}
      \centering
      PSNR\\[0.18cm]
      true coded bitrate\\[0.18cm]
      measured with codec calls
    \end{exampleblock}
  \end{columns}
\end{frame}
...
\begin{frame}{Runtime Evaluation}
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.36\textwidth}
    \begin{block}{Main Variant Runtime}
      \centering
      \scriptsize
      \begin{tabular}{lrrr}
        \toprule
    ...
    \begin{alertblock}{Interpretation}
      \scriptsize
      \begin{itemize}
        \item SE overhead: \(+0.043\%\) (\(B\)) and \(+0.086\%\) (\(C\)).
        \item Project variants stay within \(164\) to \(169\) ms/image.
        \item MBT 2018 / Cheng 2020 are \(16.1\times\) / \(16.9\times\) slower.
      \end{itemize}
    \end{alertblock}
  \end{columns}
\end{frame}
...
\begin{frame}{Discussion}
  \begin{alertblock}{Why SE Did Not Help Much Here}
    \small
    \begin{itemize}
      \item No visual difference: all models look identical at quality level 3.
      \item The global blanket: one channel weight scales every pixel in that channel.
      \item The background bottleneck: flat background regions get scaled up too, which can waste bits.
      \item Cheng 2020 uses a 3D attention map, so flat areas and detailed regions can be weighted differently.
    \end{itemize}
  \end{alertblock}
\end{frame}
...
\begin{frame}{Conclusion}
  ...
  \vspace{0.35cm}
  \begin{exampleblock}{If I Continued This Project}
    Run multiple seeds, and test richer attention placement or stronger entropy models.
  \end{exampleblock}
\end{frame}
