...
The runtime analysis in this report is strictly an evaluation/inference study. We report GPU forward-pass time together with CPU \texttt{compress()}/\texttt{decompress()} timing, because that is how the final evaluation code measures the practical inference path. We do not report total training time, epoch timing, or GPU-utilization summaries in the final comparison, even though the repository contains other training diagnostics, because those measurements were not part of the cleaned runtime study.
