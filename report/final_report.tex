...
\begin{tikzpicture}[
  ...
  se/.style={draw, rounded corners=3pt, align=center, minimum width=1.4cm, minimum height=0.75cm, fill=orange!18},
  arr/.style={-{Latex[length=2.4mm]}, thick},
  node distance=0.65cm,
]
\node[box] (in) {Input};
\node[box, below=of in] (c1) {Conv + GDN};
\node[box, below=of c1] (c2) {Conv};
\node[se, below=of c2] (se) {SE};
\node[box, below=of se] (c3) {Conv + GDN};
...
\draw[arr] (c3) -- (out);
\end{tikzpicture}
\caption{Encoder-side view. In the attention variants, one SE block is inserted between the middle encoder stages.}
...
\node[box, below=of d1] (d2) {Deconv};
\node[se, below=of d2] (se) {SE};
\node[box, below=of se] (d3) {Deconv + IGDN};
...
\draw[arr] (d3) -- (out);
\end{tikzpicture}
\caption{Decoder-side view. Only variant \(C\) adds the highlighted decoder SE block.}
...
\begin{tikzpicture}[
  font=\small,
  row/.style={draw, rounded corners=3pt, minimum width=1.55cm, minimum height=0.75cm, align=center, fill=blue!6},
  se/.style={draw, circle, minimum size=0.42cm, fill=orange!18, inner sep=0pt},
  arr/.style={-{Latex[length=2.3mm]}, thick},
  node distance=0.8cm and 0.45cm,
]
\node at (0,0.95) {\(A, A_{\mathrm{ft}}\)};
\node[row] (a1) at (2.2,0.95) {Encoder};
\node[row] (a2) at (4.2,0.95) {Prior};
\node[row] (a3) at (6.2,0.95) {Decoder};
\draw[arr] (a1) -- (a2);
\draw[arr] (a2) -- (a3);

\node at (0,0) {\(B\)};
\node[row] (b1) at (2.2,0) {Encoder};
\node[se] (bs) at (3.15,0) {\scriptsize SE};
\node[row] (b2) at (4.2,0) {Prior};
\node[row] (b3) at (6.2,0) {Decoder};
\draw[arr] (b1) -- (bs);
\draw[arr] (bs) -- (b2);
\draw[arr] (b2) -- (b3);

\node at (0,-0.95) {\(C\)};
\node[row] (c1) at (2.2,-0.95) {Encoder};
\node[se] (cs1) at (3.15,-0.95) {\scriptsize SE};
\node[row] (c2) at (4.2,-0.95) {Prior};
\node[row] (c3) at (6.2,-0.95) {Decoder};
\node[se] (cs2) at (7.15,-0.95) {\scriptsize SE};
\draw[arr] (c1) -- (cs1);
\draw[arr] (cs1) -- (c2);
\draw[arr] (c2) -- (c3);
\draw[arr] (c3) -- (cs2);
\end{tikzpicture}
\caption{Variant comparison. The project modifies only the transform network: \(B\) adds encoder attention, and \(C\) adds both encoder and decoder attention.}
